...
export function TextArea({ type, name, id, required, className, placeholder }) {
  return (
    <textarea
      type={type}
      name={name}
      id={id}
      className={`bg-gray-200 border border-gray-300 text-gray-900 text-sm rounded-sm focus:outline-none focus:ring focus:border-gray-600 block w-full p-2 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500 ${className}`}
      placeholder={placeholder}
      required={required}
    />
  );
