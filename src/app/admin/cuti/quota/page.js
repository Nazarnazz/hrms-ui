...
import { Label } from "@/app/components/admin/input";
...
export default function Setting() {
  ...
  //tombol + dan - pada modal edit
  const [quota, setQuota] = useState(18);
  ...
  const handleDecrease = () => {
    // tambah validasi agar tidak negatif
    setQuota((prev) => (prev > 0 ? prev - 1 : 0));
  };

  const handleChange = (e) => {
    const value = e.target.value;
    // Pastikan hanya angka positif atau kosong
    if (value === "" || /^[0-9]+$/.test(value)) {
      setQuota(Number(value));
    }
  };
                      ...
                      <TableRow>
                        ...
                        <TableHeader className="text-center">
                          <div className="flex items-center">
                            Nama
                            <svg onClick={() => handleSort("nama")} className="w-3 h-3 ms-1.5 hover:text-blue-900 dark:hover:text-gray-300" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="grid grid-cols-8 gap-4">
            ...
            <div className="col-span-2">
              ...
              <div className="w-full bg-gray-800 rounded py-1 border border-gray-600 grid grid-cols-3">
                <button type="button" onClick={handleDecrease} className="items-center text-center hover:bg-gray-700 rounded">
                  -
                </button>
                <input
                  type="number"
                  ...
                  className="bg-transparent text-center w-full outline-none appearance-none
                    [&::-webkit-inner-spin-button]:appearance-none
                    [&::-webkit-outer-spin-button]:appearance-none
                    [-moz-appearance:textfield]"
                  min="0"
                />
                <button type="button" onClick={handleIncrease} className="items-center text-center text-white hover:bg-gray-700 rounded">
                  +
                </button>
              </div>
            </div>
