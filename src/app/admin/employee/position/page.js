...
export default function Position() {
  ...
  const filteredAndSortedPosition = [...Position]
    .filter((item) => {
  ...
  const paginatedPosition = filteredAndSortedPosition.slice(startIndex, endIndex);
  const totalPages = Math.ceil(filteredAndSortedPosition.length / itemsPerPage);
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      setData(paginatedPosition); // Ambil dari data lokal
    ...
    <>
      <Layout>
        <div className="p-4 mt-12">
          <Breadcrumb>/ Employees / Positions</Breadcrumb>
          {isLoading ? (
            ...
            <div>
              <span className="text-[20px] mt-4 ">Positions</span>
      ...
      <Dialog open={isAddOpen} onClose={() => setIsAddOpen(false)}>
        ...
        <DialogBody>
          <fieldset className="mt-2 py-5 flex items-center gap-6">
            <Label htmlFor="name">Nama Position</Label>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              <fieldset>
                ...
                <Input type="text" name="position" id="position" className="ps-4" placeholder="MPP-PKK" required={true} />
              </fieldset>
            </div>
            <div>
              <fieldset>
